...
public class ObjectSpawner1 : MonoBehaviour
{
    ...
    void Update()
    {
        if(Input.touchCount > 0 && Input.touches[0].phase == TouchPhase.Began)
        {
            ...
            {
                DrawPipe(pipeCyllinder, placementIndicator.transform.position.x, placementIndicator.transform.position.z, pipes.pipe[i].length, pipes.pipe[i].axis);
            }
        }
    ...
    {
        cyllinder = Instantiate(cyllinder, new Vector3(x, placementIndicator.transform.position.y - 1.5f, z), Quaternion.Euler(90, 90 * System.Convert.ToInt32(axis == "x"), 0));
